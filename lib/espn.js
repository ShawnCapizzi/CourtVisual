you, you'd, you again all stacked - calling out now cause // CourtVisual — ESPN enrichment (server-only, used by /api/games).
//
...
const SPORT = { nba: ["basketball", "nba"], mlb: ["baseball", "mlb"], nhl: ["hockey", "nhl"], nfl: ["football", "nfl"], wnba: ["basketball", "wnba"], mls: ["soccer", "usa.1"] };
...
export async function getLeagueContext(league) {
  ...
  return {
    ...
    record(name) { const id = matchId(name); const r = id ? standing.get(id) : null; if (!r || r.wins == null || r.losses == null) return null; return { wins: r.wins, losses: r.losses, str: `${r.wins}\u2013${r.losses}` }; },
    contention(name) { const id = matchId(name); return id ? contendFromPct(standing.get(id)?.pct) : null; },
    winPct(name) { const id = matchId(name); const p = id ? standing.get(id)?.pct : null; return p == null ? null : p; },
    storyline(name) {
