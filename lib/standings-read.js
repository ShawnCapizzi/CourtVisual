...
// Compact rank for the header glance ("5th") and rank-with-group for the drawer
// ("5th NLE"). Group falls back to nothing when a league has no clean abbreviation,
// so we degrade to the bare rank rather than print something broken.
const ORD = (n) => { const s = ["th", "st", "nd", "rd"], v = n % 100; return n + (s[(v - 20) % 10] || s[v] || s[0]); };
const pickRank = (row) => row.division_rank ?? row.conference_rank ?? row.league_rank ?? row.playoff_seed ?? null;
export function rankOnly(row) {
  if (!row) return null;
  const r = pickRank(row);
  return r == null ? null : ORD(r);
}
export function rankGroup(row) {
  if (!row) return null;
  const r = pickRank(row);
  if (r == null) return null;
  const grp = row.division_abbr || row.conference_abbr || null;
  return grp ? `${ORD(r)} ${grp}` : ORD(r);
}
