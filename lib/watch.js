...
const GUIDES = {
  ...
  worldcup: {
    national: [{ label: "FOX / FS1 (English)" }, { label: "Telemundo / Universo (Spanish)" }],
    streamer: { key: "foxone", label: "FOX One", url: "https://www.foxsports.com", note: "all 104 matches in English" },
    localNote: "2026 World Cup: every match airs on FOX/FS1 (English) and Telemundo/Universo (Spanish). Spanish streams on Peacock; some matches free on Tubi.",
  },
  soccer: {
    national: [{ label: "Varies by competition" }],
    streamer: null,
    localNote: "Soccer rights vary by league and match, so check the event broadcaster.",
  },
  olympics: {
...
// Some surfaces pass a Ticketmaster genre ("baseball", "hockey") instead of a league key.
// Map the unambiguous ones onto the right guide. Ambiguous genres (basketball, football
// could be pro or college) stay on the honest default.
const ALIASES = { baseball: "mlb", hockey: "nhl", "ice hockey": "nhl", futbol: "soccer" };

// Resolve watch options for a game. `league` may be a league key, a TM genre, a feed id, or null.
export function watchOptions(league, game) {
  const key = (league || "").toLowerCase();
  const guide = GUIDES[ALIASES[key] || key] || DEFAULT_GUIDE;
...
// Where to actually watch a given network/label online. Keyword-matched so any label
// (compound like "ABC / ESPN" or annotated like "FOX / FS1 (Sat)") resolves to a real
// streaming/live destination. Matched on whole words (so "competition" never trips "ion"),
// first hit wins, and streaming homes are ordered ahead of their linear-TV parents so
// cord-cutters land on the watchable app. Unknown/vague slots return null and stay info-only.
const WATCH_LINKS = [
  ...
  ["ion", "https://www.iontelevision.com"],
  ["nbc", "https://www.nbc.com/live"],
  ["cbs", "https://www.paramountplus.com"],
];

export function watchLink(label) {
  const s = " " + String(label || "").toLowerCase().replace(/[^a-z0-9]+/g, " ").trim() + " ";
  for (const [token, url] of WATCH_LINKS) if (s.includes(" " + token + " ")) return url;
  return null;
}
