...
export function matchupFromSpread(spread) {
  ...
  const why = a <= 3 ? "going to be a close one" : a <= 7 ? "going to be a good one" : "one side's a step ahead";
  return { value: Math.round(v * 10) / 10, why };
}
export function matchupFromRecords(aPct, bPct) {
  ...
  const why = both ? "2 great teams" : gap <= 0.08 ? "evenly matched" : "one side's been better lately";
  return { value: Math.round(v * 10) / 10, why };
}
...
const CLOSE_WHY = new Set(["going to be a close one", "evenly matched"]);
const MISMATCH_WHY = new Set(["one side's a step ahead", "one side's been better lately"]);
...
export function recommend(g, w, fanCtx, voice) {
  ...
  if (score >= 7) {
    const finishOK = (g.historic || 0) >= 6 && !CLOSE_WHY.has(g.matchupWhy) && !MISMATCH_WHY.has(g.matchupWhy);
