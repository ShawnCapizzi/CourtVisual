...
export const TEAMS = [
  ...
  { label: "San Francisco Giants", name: "Giants", city: "San Francisco", slug: "sf-giants", league: "mlb", primary: "#FD5A1E", secondary: "#27251F" },
  { label: "Seattle Mariners", name: "Mariners", city: "Seattle", slug: "mariners", league: "mlb", primary: "#0C2C56", secondary: "#005C5C" },
  { label: "St. Louis Cardinals", name: "Cardinals", city: "St. Louis", slug: "stl-cardinals", league: "mlb", primary: "#C41E3A", secondary: "#0C2340" },
  ...
  { label: "Toronto Blue Jays", name: "Blue Jays", city: "Toronto", slug: "blue-jays", league: "mlb", primary: "#134A8E", secondary: "#E8291C" },
  ...
  { label: "Toronto Raptors", name: "Raptors", city: "Toronto", slug: "raptors", league: "nba", primary: "#CE1141", secondary: "#000000" },
  ...
  { label: "Calgary Flames", name: "Flames", city: "Calgary", slug: "flames", league: "nhl", primary: "#C8102E", secondary: "#F1BE48" },
  ...
  { label: "Edmonton Oilers", name: "Oilers", city: "Edmonton", slug: "oilers", league: "nhl", primary: "#00205B", secondary: "#CF4520" },
  { label: "Florida Panthers", name: "Panthers", city: "Sunrise", slug: "fla-panthers", league: "nhl", primary: "#041E42", secondary: "#C8102E" },
  { label: "Los Angeles Kings", name: "Kings", city: "Los Angeles", slug: "la-kings", league: "nhl", primary: "#111111", secondary: "#A2AAAD" },
  { label: "Minnesota Wild", name: "Wild", city: "Saint Paul", slug: "wild", league: "nhl", primary: "#154734", secondary: "#A6192E" },
  { label: "Montreal Canadiens", name: "Canadiens", city: "Montreal", slug: "canadiens", league: "nhl", primary: "#A6192E", secondary: "#001E62" },
  ...
  { label: "New York Rangers", name: "Rangers", city: "New York", slug: "ny-rangers", league: "nhl", primary: "#0038A8", secondary: "#CE1126" },
  { label: "Ottawa Senators", name: "Senators", city: "Ottawa", slug: "senators", league: "nhl", primary: "#C8102E", secondary: "#010101" },
  ...
  { label: "Toronto Maple Leafs", name: "Maple Leafs", city: "Toronto", slug: "maple-leafs", league: "nhl", primary: "#00205B", secondary: "#FFFFFF" },
  { label: "Utah Mammoth", name: "Mammoth", city: "Salt Lake City", slug: "mammoth", league: "nhl", primary: "#6CACE4", secondary: "#010101" },
  { label: "Vancouver Canucks", name: "Canucks", city: "Vancouver", slug: "canucks", league: "nhl", primary: "#00205B", secondary: "#00843D" },
  ...
  { label: "Winnipeg Jets", name: "Jets", city: "Winnipeg", slug: "wpg-jets", league: "nhl", primary: "#041E42", secondary: "#004C97" },
  ...
  { label: "CF Montréal", name: "CF Montréal", city: "Montréal", slug: "cf-montreal", league: "mls", primary: "#003087", secondary: "#000000" },
  ...
  { label: "Toronto FC", name: "Toronto FC", city: "Toronto", slug: "toronto-fc", league: "mls", primary: "#D71920", secondary: "#FFFFFF" },
  { label: "Vancouver Whitecaps FC", name: "Vancouver Whitecaps FC", city: "Vancouver", slug: "vancouver-whitecaps-fc", league: "mls", primary: "#00245E", secondary: "#FFFFFF" },
  ...
  { label: "Toronto Tempo", name: "Tempo", city: "Toronto", slug: "tempo", league: "wnba", primary: "#612C51", secondary: "#B8CCEA" },
...
// never have to migrate. Two slots were repurposed without renaming their key:
// `historic` is MATCHUP (competitiveness, priced by spread). `hot` is now THE RACE,
// the standings pressure around the game (contention), NOT the dead "stars" signal it
// used to hold. Stars was a flat baseline we never measured; the race is live data.
export const FACTORS = [
  { key: "playoff", label: "Stakes" },
  { key: "rivalry", label: "Rivalry" },
  { key: "hot", label: "The race" },
...
export const PRESETS = [
  ...
  { id: "stars", label: "In the hunt", w: { playoff: 20, rivalry: 15, hot: 50, historic: 15 } },
...
const SAMPLE_TEMPLATES = [
  { tag: "Rivalry night", date: "Fri · 7:30 PM", ds: "11-14", home: true, playoff: 7, rivalry: 10, hot: 8, historic: 7 },
  { tag: "Marquee matchup", date: "Sat · 8:00 PM", ds: "11-15", home: true, playoff: 6, rivalry: 6, hot: 6, historic: 9 },
  { tag: "Division game", date: "Wed · 7:00 PM", ds: "11-19", home: false, playoff: 8, rivalry: 8, hot: 9, historic: 6 },
  { tag: "Weeknight game", date: "Tue · 7:00 PM", ds: "11-25", home: true, playoff: 5, rivalry: 5, hot: 5, historic: 6 },
];
...
// Big games carry a floor by stage; the race, top-rivalry, and history
...
export function scoreParts(g, w) {
  ...
  // Per-factor share of the base, so the recommendation sentence can name the real
  // drivers. Additive only: existing callers ignore it, base/score are untouched.
  const contrib = {
    playoff:  (w.playoff  * g.playoff)  / sum,
    rivalry:  (w.rivalry  * g.rivalry)  / sum,
    hot:      (w.hot      * g.hot)      / sum,
    historic: (w.historic * g.historic) / sum,
  };
  return { base: Math.round(base * 10) / 10, floor: Math.round(floor * 10) / 10, floored: floor > base, score, contrib };
}
export function scoreOf(g, w) { return scoreParts(g, w).score; }

// ---- Fan lens ----
// When the user views through their fandom, their own teams' games carry honest,
// DISCLOSED bumps on top of the neutral score. Three stacking sources, all from
// data we already have. Applied AFTER the stakes floor, clamped to 10. A flippable,
// labeled lens — never a silent skew. ctx = { intensity:"follow"|"diehard",
// isRival:boolean, contention:number|null }.
const FAN_TEAM = { follow: 0.4, diehard: 1.5 };
const FAN_RIVAL = { follow: 0.25, diehard: 0.5 };
const FAN_RACE = { follow: 0.15, diehard: 0.4 };
export function fanBump(g, ctx) {
  if (!ctx || !ctx.intensity) return { bump: 0, reasons: [] };
  const i = ctx.intensity === "diehard" ? "diehard" : "follow";
  let bump = 0; const reasons = [];
  bump += FAN_TEAM[i]; reasons.push(`Your team +${FAN_TEAM[i]}`);
  if (ctx.isRival) { bump += FAN_RIVAL[i]; reasons.push(`Your rival +${FAN_RIVAL[i]}`); }
  if (ctx.contention != null && ctx.contention >= 7) { bump += FAN_RACE[i]; reasons.push(`In the race +${FAN_RACE[i]}`); }
  return { bump: Math.round(bump * 10) / 10, reasons };
}
// Final fan-view score: neutral score + disclosed bump, clamped to 10.
export function fanScoreOf(g, w, ctx) {
  return Math.round(Math.min(10, scoreOf(g, w) + fanBump(g, ctx).bump) * 10) / 10;
}

// ---- Matchup (competitiveness) ----
// How good a game this is likely to be, independent of stakes/stars/rivalry.
// Best signal first: the market line (it prices in injuries, rest, and form), then a
// records gap, then a plain neutral when we cannot assess. The market line is used only
// as a hidden input. The copy never shows odds or bettor language, in keeping with
// attend-not-bet. Returns { value 0-10, why } for the card to disclose the read.
export function matchupFromSpread(spread, spreadTeam, oppTeam) {
  if (spread == null || isNaN(spread)) return null;
  const a = Math.abs(spread);
  const v = Math.max(2, Math.min(10, 10 - a * 0.6));
  // `spread` is spreadTeam's line: negative = spreadTeam favored. Name the favorite when lopsided.
  let why, kind;
  if (a <= 3) { why = "going to be a close one"; kind = "close"; }
  else if (a <= 7) { why = "going to be a good one"; kind = "good"; }
  else {
    const fav = spread < 0 ? spreadTeam : oppTeam;
    why = fav ? `${fav} favored on paper` : "one side's favored on paper";
    kind = "mismatch";
  }
  return { value: Math.round(v * 10) / 10, why, kind };
}
export function matchupFromRecords(aPct, bPct, aName, bName) {
  ...
  let why, kind;
  if (aPct >= 0.55 && bPct >= 0.55) { why = "two strong sides"; kind = "both"; }
  else if (gap <= 0.08) { why = "evenly matched"; kind = "even"; }
  else {
    const better = aPct >= bPct ? aName : bName;
    why = better ? `${better} have the better record` : "one side has the better record";
    kind = "mismatch";
  }
  return { value: Math.round(v * 10) / 10, why, kind };
}
export const NEUTRAL_MATCHUP = { value: 5, why: null };
export const verdict = (s) => (s >= 9.3 ? "Hottest ticket" : s >= 8.5 ? "Must see" : s >= 7 ? "Highly recommended" : s >= 5.5 ? "Worth attending" : "Good game");

// Canonical game-time + freshness. ONE definition every surface shares, so no screen can
// re-decide (or forget) what "over" means. gameStartMs parses the iso instant (null when a game
// is undated/TBD). isFinished is deliberately conservative: an explicit "post" status, or a
// kickoff long enough ago that no sport is still playing. Undated games are never "finished".
export function gameStartMs(g) {
  if (!g || !g.iso) return null;
  const t = new Date(g.iso).getTime();
  return Number.isNaN(t) ? null : t;
}
export function isFinished(g, now = Date.now()) {
  if (g && g.status === "post") return true;
  const t = gameStartMs(g);
  return t !== null && t < now - 5 * 3600 * 1000;
}

// ---- Recommendation sentence ----
// Built from the SAME factor math as the score, so the words never disagree with the
// number. Wording is varied with a per-game seed (same game reads the same every time;
// different games vary across the slate). The VOICE is an announcer-inspired phrasing
// skin: it flavors the opener, the rivalry phrasing, and a closing "finish" line (all
// tone, never claims), while the factual clauses (matchup, stakes, slot) stay gated by
// real data so a voice can never make us assert something untrue. fanCtx makes it
// lens-aware. No odds, no bettor language, no em dashes, numerals not spelled out.

// Shared, voice-neutral, data-gated clauses.
const STAKES_POOL = {
  Championship: ["a title on the line", "everything on the line"],
  "Knockout stage": ["win or go home", "lose and you're done"],
  Playoffs: ["playoff stakes", "playoff intensity"],
  real: ["real stakes", "standings on the line"],
};
const SLOT_HOOKS = { // Mon/Tue/Thu omitted on purpose: no hook, stays quiet.
  fri: ["TGIF, it's game night", "start the weekend off right"],
  sat: ["make a Saturday of it", "a proper Saturday night out"],
  sun: ["perfect Sunday plans", "a Sunday well spent"],
  wed: ["hump night", "a midweek lift"],
};

// Announcer-inspired voices. {name} is filled with the rivalry name when there is one,
// and those entries are skipped when there isn't. open = everyday opener, rare = the ~1
// in 9 surprise, elite = reserved for 9+ games, finish = a closing line that only fires
// when the matchup is projected competitive.
const VOICES = {
  // House is the app's own voice: the liveliest, most action-forward blend, set as default.
  house: {
    label: "House",
    open: ["Worth the watch."],
    rare: ["Don't miss this one.", "Keep this one on.", "Circle this one.", "This one's got juice."],
    elite: ["This is the one.", "Cancel your plans.", "Clear your calendar.", "Appointment viewing."],
    rivalry: ["{name} night", "rivalry heat", "no love lost", "bragging rights on the line"],
    finish: ["could come down to the end", "expect a few big swings", "one play could decide it"],
  },
  scully: {
    label: "Warm (Scully)",
    open: ["Settle in.", "Worth the watch."],
    rare: ["Stay until the end.", "The kind of night people talk about after."],
    elite: ["Settle in for something memorable.", "An occasion, not just a game."],
    rivalry: ["{name} night", "two proud teams", "enough history to make it matter"],
    finish: ["a finish that could sneak up on you", "the kind that rewards a patient fan"],
  },
  cosell: {
    label: "Bold (Cosell)",
    open: ["This is not background noise.", "Worth the watch."],
    rare: ["A measuring-stick game.", "Reputation against reality."],
    elite: ["Appointment viewing.", "Where the real ones separate."],
    rivalry: ["{name} night", "two teams with something to prove", "no hiding place here"],
    finish: ["and close games reveal the truth", "with real consequences"],
  },
  madden: {
    label: "Fun (Madden)",
    open: ["Good game alert.", "Worth the watch."],
    rare: ["Big-play potential here.", "This one's got it."],
    elite: ["This is the one.", "Don't miss this one."],
    rivalry: ["{name} night", "rivalry, and somebody's making a big play", "two teams that match up"],
    finish: ["somebody's going to make a big play", "could come down to one stop"],
  },
  michaels: {
    label: "Big-event (Michaels)",
    open: ["Big spot tonight.", "Worth the watch."],
    rare: ["This one feels bigger than the date.", "Appointment viewing."],
    elite: ["This is the one.", "Own-the-night material."],
    rivalry: ["{name} night", "city bragging rights", "two strong teams"],
    finish: ["could turn on one moment", "decided late"],
  },
  costas: {
    label: "Smart (Costas)",
    open: ["A meaningful one.", "Worth the watch."],
    rare: ["Real standings value here.", "This one says where both teams stand."],
    elite: ["A game that could define a season.", "This one matters beyond tonight."],
    rivalry: ["{name} night", "geography, pride, and pressure", "both with something to prove"],
    finish: ["where momentum and timing meet", "the kind that rewards a patient fan"],
  },
  nantz: {
    label: "Classy (Nantz)",
    open: ["Settle in.", "Worth the watch."],
    rare: ["A little extra polish tonight.", "Appointment viewing."],
    elite: ["A night to build the day around.", "This one has the feel."],
    rivalry: ["{name} night", "the city and the spotlight together", "two strong teams"],
    finish: ["the details should matter", "a night that could finish beautifully"],
  },
  jackson: {
    label: "Folksy (Jackson)",
    open: ["Pull up a chair.", "Worth the watch."],
    rare: ["A little thunder in this one.", "Somebody's going to have to earn it."],
    elite: ["A big one, full heart.", "This is the one."],
    rivalry: ["{name} night", "a backyard argument with uniforms", "rivalry heat and a ready crowd"],
    finish: ["somebody's going to have to earn it", "could get interesting fast"],
  },
  marv: {
    label: "Electric (Albert)",
    open: ["Keep this one on.", "Worth the watch."],
    rare: ["Sneaky-good matchup.", "This one's got juice."],
    elite: ["Built for big moments.", "This is the one."],
    rivalry: ["{name} night", "rivalry juice", "a crowd ready to explode"],
    finish: ["expect runs and counters", "comeback potential all over it"],
  },
  caray: {
    label: "Fan-first (Caray)",
    open: ["Put this one on.", "Worth the watch."],
    rare: ["Keep the remote close.", "This is a fun one."],
    elite: ["Cancel your plans.", "Clear the night for this."],
    rivalry: ["{name} night", "plenty to argue about", "rivalry heat"],
    finish: ["so keep the remote close", "and it could get loud"],
  },
  buck: {
    label: "Clean (Buck)",
    open: ["Worth the watch.", "Simple call: worth it."],
    rare: ["One of the better games on the board.", "Clean reason to watch."],
    elite: ["This is the one.", "Best game on the board tonight."],
    rivalry: ["{name} night", "bragging rights in play", "two good teams"],
    finish: ["should come down to the end", "points to a close one"],
  },
};
export const VOICE_LIST = Object.entries(VOICES).map(([id, v]) => ({ id, label: v.label }));
const VOICE_IDS = Object.keys(VOICES);
// Matchup clauses that already speak to closeness or a mismatch: a finish line about a
// close finish would either repeat them or contradict them, so it is suppressed for these.
const MISMATCH_KINDS = new Set(["close", "even", "mismatch"]);

const dayKey = (date) => String(date || "").trim().slice(0, 3).toLowerCase();
const capFirst = (s) => (s ? s.charAt(0).toUpperCase() + s.slice(1) : s);

// Stable per-game seed (FNV-1a over opponent + date). Same game, same wording every time.
function seedOf(g) {
  const s = `${g.oppSlug || g.opp || ""}|${g.date || ""}`;
  let h = 2166136261;
  for (let i = 0; i < s.length; i++) { h ^= s.charCodeAt(i); h = Math.imul(h, 16777619); }
  return h >>> 0;
}
const pick = (arr, seed, salt) => arr[((seed ^ Math.imul(salt, 2654435761)) >>> 0) % arr.length];

function leadClause(g, V, seed) {
  if (g.topRivals) {
    let pool = V.rivalry && V.rivalry.length ? V.rivalry : ["{name} night"];
    if (!g.rivalryName) pool = pool.filter((p) => !p.includes("{name}"));
    if (!pool.length) pool = ["rivalry night"];
    return pick(pool, seed, 11).replace("{name}", g.rivalryName || "");
  }
  if (STAKES_POOL[g.tag]) return pick(STAKES_POOL[g.tag], seed, 13);
  if ((g.playoff || 0) >= 7) return pick(STAKES_POOL.real, seed, 13);
  return null;
}
function slotHook(g, seed) {
  const pool = SLOT_HOOKS[dayKey(g.date)];
  return pool ? pick(pool, seed, 17) : null;
}
function openerFor(V, score, seed) {
  const base = V.open && V.open.length ? V.open : ["Worth the watch."];
  if (score >= 9.3 && V.elite && V.elite.length && seed % 4 === 0) return pick(V.elite, seed, 19);
  if (V.rare && V.rare.length && seed % 9 === 0) return pick(V.rare, seed, 19);
  return pick(base, seed, 20);
}

// voice: a VOICE id, or "mix"/undefined to rotate a stable voice per game.
export function recommend(g, w, fanCtx, voice) {
  if (!g) return null;
  const parts = scoreParts(g, w);
  const fb = fanCtx ? fanBump(g, fanCtx) : { bump: 0 };
  const score = Math.round(Math.min(10, parts.score + (fb.bump > 0 ? fb.bump : 0)) * 10) / 10;
  const seed = seedOf(g);
  const V = VOICES[voice] || VOICES[pick(VOICE_IDS, seed, 23)];

  const lead = leadClause(g, V, seed);
  const matchup = g.matchupWhy || null;

  if (score >= 7) {
    const finishOK = (g.historic || 0) >= 6 && !MISMATCH_KINDS.has(g.matchupKind);
    const finish = finishOK ? pick(V.finish, seed, 29) : null;
    const core = [lead, matchup, finish].filter(Boolean).join(", ");
    const hook = slotHook(g, seed);
    let out = openerFor(V, score, seed);
    if (core) out += ` ${capFirst(core)}.`;
    if (hook) out += ` ${capFirst(hook)}.`;
    return out;
  }
  // Not earned: a quiet, honest read with no sell and no hook.
  const core = [lead, matchup].filter(Boolean).join(", ");
  return core ? `${capFirst(core)}.` : null;
}
