import { rivalryFactor, isTopRivalry } from "./rivalries";
...
export function sampleSlate(team) {
  ...
  return SAMPLE_TEMPLATES.map((tpl, i) => {
    ...
    return { ...tpl, opp: opp.name, oppSlug: opp.slug, rivalry: rivalryFactor(team.name, opp.name), topRivals: isTopRivalry(team.name, opp.name), url: null, minPrice: null, venue: null };
  });
}
...
export function scoreParts(g, w) {
  ...
  if (floor) floor += Math.max(0, (g.hot || 0) - 7) * 0.15 + (g.topRivals ? 0.3 : 0) + ((g.historic || 0) >= 9 ? 0.2 : 0);
